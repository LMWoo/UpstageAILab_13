...
class HNMCallback(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        MAX_AUG_PER_CLASS = {3: 10, 7: 10}
        DEFAULT_MAX_AUG   = 1

        def max_aug_for(cls: int) -> int:
            return MAX_AUG_PER_CLASS.get(cls, DEFAULT_MAX_AUG)

        # ── Hard-Negative 후보 선정 ───────────────────────────────
        topk = 3

        worst = [
            cls for cls, _ in sorted(per_cls.items(),
                                    key=lambda kv: kv[1], 
                                    reverse=True)
            if self.aug_cnt[cls] < max_aug_for(cls)
        ][:topk]
