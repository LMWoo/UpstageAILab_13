...
### 4. 대회 회고
 - 이번 딥러닝 경진대회를 통해 이미지 분류와 문서 분류를 포함한 다양한 모델 실험과 전처리, 앙상블 등 전체 딥러닝 파이프라인을                            
직접 경헙해볼 수 있었습니다. 처음 딥러닝 접한 팀원도 있었지만, 각자 빠르게 적응하며 augmentation, EDA, 실험 설계의 중요성에 대해 체감하였습니다.
 - 중간에 **서버 장애 이슈**를 겪으면서, 실험 결과와 모델을 체계적으로 관리하는 아티팩트 관리의 중요성을 절실히 느꼈습니다.
